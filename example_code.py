...
# Call the getSelf() method from the Organization Class 
# Extract the Mist org_id for future use in the script
mist_self = dashboard.organization.getSelf()
org_id = mist_self["privileges"][0]["org_id"]


# Call the getSites() method from the Organization Class
# Extract the required Mist site_id for future use in the script
mist_site = dashboard.organization.getSites(org_id)
site_id = mist_site[0]["id"]


# get RFTemplate & NetworkTemplate for site creation later in script
rftemp = dashboard.organization.getRFTemplates(org_id)
rftemplate_id = rftemp[0]["id"]
nwtemp = dashboard.organization.getNetworkTemplates(org_id)
networktemplate_id = nwtemp[0]["id"]


# Create a Mist Site
create_examplesite = dashboard.organization.createSite(org_id, 
                                                "Test Site by API", 
