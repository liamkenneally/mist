...
from .config import (
    ...
    SUPPRESS_LOGGING, SIMULATE_API_CALLS, BE_GEO_ID, MIST_PYTHON_SDK_CALLER
)
...
class DashboardAPI(object):
    """
    **Creates a persistent Mist dashboard API session**
    - api_key (string): API key generated in dashboard; can also be set as an environment variable MIST_DASHBOARD_API_KEY
    ...
    - caller (string): optional identifier for API usage tracking; can also be set as an environment variable MIST_PYTHON_SDK_CALLER
    ...
    def __init__(self, api_key=None, base_url=DEFAULT_BASE_URL, single_request_timeout=SINGLE_REQUEST_TIMEOUT,
                 ...
                 be_geo_id=BE_GEO_ID, caller=MIST_PYTHON_SDK_CALLER, use_iterator_for_get_pages=False):
        # Check API key
        ...
        caller = caller or os.environ.get('MIST_PYTHON_SDK_CALLER')
